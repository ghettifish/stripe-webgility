...
/**
 * WB_Stripe_Customer class.
 *
 * Represents a Stripe Customer.
 */
class WB_Stripe_Customer {
	...
	public function create_customer( $args = array() ) {
		...
		$defaults['metadata'] = apply_filters( 'wb_stripe_customer_metadata', $metadata, $user );

		$args     = wp_parse_args( $args, $defaults );
		$response = WB_Stripe_API::request( apply_filters( 'wb_stripe_create_customer_args', $args ), 'customers' );
	...
	public function add_source( $source_id, $retry = true ) {
		...
		$response = WB_Stripe_API::request(
			array(
				'source' => $source_id,
			),
	...
	public function delete_source( $source_id ) {
		...
		$response = WB_Stripe_API::request( array(), 'customers/' . $this->get_id() . '/sources/' . sanitize_text_field( $source_id ), 'DELETE' );

		$this->clear_cache();

		if ( empty( $response->error ) ) {
			do_action( 'wb_stripe_delete_source', $this->get_id(), $response );
	...
	public function set_default_source( $source_id ) {
		...
		if ( empty( $response->error ) ) {
			do_action( 'wb_stripe_set_default_source', $this->get_id(), $response );
